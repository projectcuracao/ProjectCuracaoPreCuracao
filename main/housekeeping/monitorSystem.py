...
def  monitorSystem(source, delay):
	...
	if (fanOn == False): 
		...
		# check to see if solar voltage is > 5.0 on pi



		if (solarvoltage > 5.0):
			print "solar voltage > 5.0"
			...
			# no point if solar voltage on Pi power supply is below 5V
	...
	if (hardwareactions.returnFanState()== False):
		...
		if (solarvoltage <= 5.0):
			hardwareactions.setfan(False)
			pclogging.log(pclogging.INFO, __name__, "fanOFF solarvoltage =< 5.0 ")
